...
function ThumbTrack({
  ...
  const calculate = useCallback(
    (items: Primitive.TOCItemInfo[], previous: ThumbBoxInfo | null) => {
      const out: Record<string, string> = {};
      const startIdx = items.findIndex((item) => item.active);
      if (startIdx === -1) return { styles: out, next: previous };
      ...
      if (thumbBox) {
        ...
        return { styles: out, next };
      }

      return { styles: out, next: previous };
    },
    [computed, thumbBox],
  );
  ...
  const applyStyles = useCallback(
    (items: Primitive.TOCItemInfo[]) => {
      ...
      const { styles, next } = calculate(items, previousRef.current);
  ...
  Primitive.useTOCListener(applyStyles);

  useEffect(() => {
    applyStyles(tocInfo.get());
  }, [applyStyles, tocInfo]);
